class Block extends THREE.Mesh {
  ...
  update(delta) {
    ...
    if (this.velocity.length() < 0.2) this.velocity = new THREE.Vector3();
    ...
    if (this.velocity.length() > 0) {
      this.applyForce(
        this.velocity
          .clone()
          .normalize()
          .multiplyScalar(-Settings.AirResistance)
      );
      if (this.position.y == this.bottom)
        this.applyForce(
          this.velocity
            .clone()
            .normalize()
            .multiplyScalar(-Settings.Friction)
        );
    }

    //grawitacja
    if (this.position.y > this.bottom)
      this.acceleration.push(new THREE.Vector3(0, -Settings.Gravity, 0));
    ...
    if (this.velocity.length() > 0.2)
      this.position.add(this.velocity.clone().multiplyScalar(delta / 240));
    ...
    if (this.position.distanceTo(other.position) < 10) {
      if (this.position.y - other.position.y > 0) {
        ...
        this.acceleration.push(
          new THREE.Vector3(0, -fallV + Settings.Gravity, 0)
        );
      }
    }
  }
