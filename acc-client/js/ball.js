class Ball extends THREE.Mesh {
  ...
  update(delta) {
    if (this.flying) {
      ...
      let y =
        this.startingVelocity * this.flyTime * Math.sin(this.angle) -
        (Settings.Gravity * this.flyTime * this.flyTime) / 2;
      let z = this.startingVelocity * this.flyTime * Math.cos(this.angle);
      let oldPos = this.position.clone();
      this.position.set(
        this.startX + z * this.dirVector.x,
        this.startY + y,
        this.startZ + z * this.dirVector.z
      );
      ...
      if (this.position.y <= 0) {
        ...
        this.placeInBarrel(
          ...
          15,
          -(this.cannon.rotation.y + Math.PI / 2),
          this.cannon.barrel.rotation.x + Math.PI / 2
        );
        if (
          this.cannon.index == Game.cannonIndex &&
          CameraControl.isFollowing &&
          CameraControl.followObj.id == this.id
        ) {
  ...
  shoot(angle, velocity) {
    if (this.flying || Game.fired) return;
    Game.fired = true;
    this.angle = angle;
  ...
  placeInBarrel(centerPos, radius, cannonAngle, barrelAngle) {
    let x =
      centerPos.x + radius * Math.cos(cannonAngle) * Math.cos(barrelAngle);
    let y = centerPos.y + radius * Math.sin(barrelAngle);
    let z =
      centerPos.z + radius * Math.sin(cannonAngle) * Math.cos(barrelAngle);
