	...
	render() {
		...
		switch (this.props.effect && this.props.effect.toLowerCase()) {
			...
			case 'nativefeedback':
				...
				effectProps['background'] = TouchableNativeFeedback.Ripple('rgba(255,255,255,1)', true)
				break
			default:
				Effect = TouchableOpacity
		}
		...
		if (Array.isArray(this.style)) {
			textstyle = this.style.map(s =>
				Object.keys(s)
					.filter(key => key.startsWith('text') || key.startsWith('font') || key == 'color')
					.reduce((reducer, key) => {
						reducer[key] = s[key]
						return reducer
					}, {})
		...
		return (
			<Effect {...effectProps} onPress={this.pressHandler} onLongPress={this.longPressHandler} disabled={this.props.disabled} style={this.style}>
				<View style={this.props.innerStyle}>{typeof this.props.children == 'string' ? <Text style={textstyle}>{this.props.children}</Text> : this.props.children}</View>
			</Effect>
		)
	}
}
